-- ============================================================
-- Migration: CRM enhancements (Dynamic calculator, task durations, workflow schedule, environment types)
-- Date: 2026-06-15
-- ============================================================

-- 1. Alterações na tabela profiles
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS modo_calculo_hora TEXT DEFAULT 'manual',
  ADD COLUMN IF NOT EXISTS nicho TEXT DEFAULT NULL;

-- Adicionar constraint de validação para modo_calculo_hora
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'profiles_modo_calculo_hora_check'
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_modo_calculo_hora_check
      CHECK (modo_calculo_hora IN ('manual', 'dinamico'));
  END IF;
END $$;

-- 2. Alterações na tabela configuracao_agenda
ALTER TABLE public.configuracao_agenda
  ADD COLUMN IF NOT EXISTS bloquear_agenda_workflow BOOLEAN DEFAULT false,
  ADD COLUMN IF NOT EXISTS permitir_conflito_interno BOOLEAN DEFAULT false;

-- 3. Alterações na tabela company_tasks
ALTER TABLE public.company_tasks
  ADD COLUMN IF NOT EXISTS duracao_minutos INTEGER DEFAULT NULL;

-- 4. Alterações na tabela eventos_agenda
ALTER TABLE public.eventos_agenda
  ADD COLUMN IF NOT EXISTS ambiente TEXT DEFAULT 'externo';

-- Adicionar constraint de validação para ambiente
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'eventos_agenda_ambiente_check'
  ) THEN
    ALTER TABLE public.eventos_agenda
      ADD CONSTRAINT eventos_agenda_ambiente_check
      CHECK (ambiente IN ('interno', 'externo'));
  END IF;
END $$;

-- Atualizar constraint de origem em eventos_agenda para aceitar 'workflow'
ALTER TABLE public.eventos_agenda
...
-- 5. Função de Verificação de Disponibilidade Atualizada com regra de concorrência interna/externa
...
BEGIN
  ...
  IF COALESCE(v_config.modo_agendamento, 'dia') = 'dia' THEN
    ...
    WHERE user_id = p_user_id
      ...
      -- Se permitir_conflito_interno for verdadeiro, ignorar eventos internos
      AND (NOT COALESCE(v_config.permitir_conflito_interno, false) OR ambiente = 'externo');
