-- ============================================================
-- Migration: Agendamento Avançado por Horários e Duração Dinâmica
-- Data: 2026-06-14
-- ============================================================
...
-- 2. Alterações na tabela configuracao_agenda
ALTER TABLE public.configuracao_agenda
  ADD COLUMN IF NOT EXISTS modo_agendamento TEXT DEFAULT 'dia',
  ADD COLUMN IF NOT EXISTS politica_bloqueio TEXT DEFAULT 'dia_inteiro',
  ADD COLUMN IF NOT EXISTS config_horarios_trabalho JSONB DEFAULT NULL;

-- Adicionar constraints de validação (com IF NOT EXISTS via bloco anônimo para segurança)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'configuracao_agenda_modo_agendamento_check'
  ) THEN
    ALTER TABLE public.configuracao_agenda 
      ADD CONSTRAINT configuracao_agenda_modo_agendamento_check 
      CHECK (modo_agendamento IN ('dia', 'hora'));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'configuracao_agenda_politica_bloqueio_check'
  ) THEN
    ALTER TABLE public.configuracao_agenda 
      ADD CONSTRAINT configuracao_agenda_politica_bloqueio_check 
      CHECK (politica_bloqueio IN ('dia_inteiro', 'por_horario'));
  END IF;
END $$;

-- 3. Alterações na tabela eventos_agenda
ALTER TABLE public.eventos_agenda
  ADD COLUMN IF NOT EXISTS horario_inicio TIME WITHOUT TIME ZONE DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS duracao_minutos INTEGER DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS horario_fim TIME WITHOUT TIME ZONE DEFAULT NULL;

-- 4. Alterações na tabela templates
ALTER TABLE public.templates
  ADD COLUMN IF NOT EXISTS slots_bloqueados JSONB DEFAULT NULL;

-- 5. Alterações na tabela datas_bloqueadas
ALTER TABLE public.datas_bloqueadas
  ADD COLUMN IF NOT EXISTS horario_bloqueado TIME WITHOUT TIME ZONE DEFAULT NULL;

-- 6. Trigger para calcular automaticamente horario_fim em eventos_agenda
CREATE OR REPLACE FUNCTION public.calculate_evento_horario_fim()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.horario_inicio IS NOT NULL AND NEW.duracao_minutos IS NOT NULL THEN
    NEW.horario_fim := (NEW.horario_inicio + (NEW.duracao_minutos || ' minutes')::interval)::time;
  ELSE
    NEW.horario_fim := NULL;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_calculate_evento_horario_fim ON public.eventos_agenda;
CREATE TRIGGER trg_calculate_evento_horario_fim
  BEFORE INSERT OR UPDATE ON public.eventos_agenda
  FOR EACH ROW
  EXECUTE FUNCTION public.calculate_evento_horario_fim();

-- 7. Função de Verificação de Disponibilidade Atualizada (check_date_availability)
-- Removemos a antiga para evitar conflitos de overload
DROP FUNCTION IF EXISTS public.check_date_availability(uuid, date);
