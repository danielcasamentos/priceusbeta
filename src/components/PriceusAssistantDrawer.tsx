  ...
  return (
    <>
      ...
      <button
        ...
        className="relative p-2.5 rounded-xl text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 transition flex items-center gap-2 group shadow-sm cursor-pointer"
      ...
      {/* 📐 DRAWER SLIDE-OVER DA DIREITA (RENDERIZADO VIA PORTAL NO BODY COM Z-INDEX MÁXIMO) */}
      {isOpen &&
        createPortal(
          <div className="fixed inset-0 z-[99999] bg-slate-950/80 backdrop-blur-sm flex justify-end animate-in fade-in duration-200">
            <div className="w-full max-w-md bg-slate-900 border-l border-slate-800 flex flex-col h-[100dvh] shadow-2xl animate-in slide-in-from-right duration-300">
                        ...
                        <div className="mt-3 pt-2.5 border-t border-slate-700/60 space-y-1.5">
                          ...
                          {m.suggestedActions.map((s, i) => (
                            <button
                              ...
                              className="w-full text-left p-2 rounded-xl bg-slate-900 hover:bg-slate-950 border border-slate-700/80 text-[11px] text-slate-200 hover:text-amber-300 transition block font-medium cursor-pointer"
                            >
                              💡 {s.label}
                            </button>
                          ))}
              ...
              {/* Input de Pergunta com Safe Area Padding para Mobile */}
              <div className="p-3 bg-slate-950 border-t border-slate-800 flex items-center gap-2 shrink-0 safe-area-pb">
                <input
                  type="text"
                  value={input}
                ...
                <button
                  onClick={() => handleSend()}
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold p-2.5 rounded-xl transition shadow-md cursor-pointer shrink-0"
                >
